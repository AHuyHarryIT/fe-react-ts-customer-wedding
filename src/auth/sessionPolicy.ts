...
const AUTH_FEEDBACK_REASON_KEY = 'customer_auth_feedback_reason';
...
export const isSessionExpiredResponse = (error: unknown): boolean => {
  if (axios.isAxiosError(error)) {
    ...
    const code = (error.response?.data as ApiErrorData | undefined)?.code;
...
export const mapForbiddenContext = (error: unknown): ForbiddenContext | null => {
  const statusCode = getErrorStatus(error);
  const data = getErrorData(error);
  ...
  const rootDetails = isRecord((data as { details?: unknown } | undefined)?.details)
    ? ((data as { details?: unknown }).details as RecordValue)
    : undefined;
